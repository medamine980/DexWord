/*! https://mths.be/iso-8859-15 v3.0.4 by @mathias | MIT license */
...
const encodingError = (mode: string) => {
	...
	// Else, `mode == 'fatal'`.
	throw new Error("encoding err");
};

// https://encoding.spec.whatwg.org/#single-byte-decoder
export const decode = (input: string | Buffer | Uint16Array, options?: DecodeOptions) => {
	let mode;
...
export const encode = (input: any, options?: EncodeOptions) => {
	let mode;
	if (options && options.mode) {
		mode = options.mode.toLowerCase();
	}
	...
	for (let index = 0; index < length; index++) {
		...
		if (INDEX_BY_CODE_POINT.has(codePoint)) {
			const pointer: any = INDEX_BY_CODE_POINT.get(codePoint);
